...
public class OrderStatisticsService {
    ...
    public boolean sendOrderServicesMail(LocalDate orderDate, String email) {
        // 해당 일자에 결재 완료된 주문들을 가져와서
        List<Order> orders = orderRepository.findOrdersBy(
                orderDate.atStartOfDay(), orderDate.plusDays(1).atStartOfDay(),
        ...
        if (!result) {
            throw new IllegalArgumentException("매출 통계 메일 작성에 실패했습니다.");
        }
